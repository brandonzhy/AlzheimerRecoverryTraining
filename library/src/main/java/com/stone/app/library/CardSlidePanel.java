...
//    private static final int VIEW_COUNT = 4;
    ...
    class MoveDetector extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            float xl = e1.getX() - e2.getX();
            float xr = e2.getX() - e1.getX();
            if (xl > FLING_MIN_DISTANCE && Math.abs(velocityX) > FLING_MIN_VELOCITY) {
                //往左滑动
                Log.i("TAG", "往左滑动");
            } else if (xr > FLING_MIN_DISTANCE && Math.abs(velocityX) > FLING_MIN_VELOCITY) {
                //往右滑动
                Log.i("TAG", "往右滑动");
            }
            return false;
...
//        int newIndex = isShowing + 4;
    ...
    private void animToSide(CardItemView changedView, int xvel, int yvel) {
        ...
        setMotionType(flyType);
    }

    private void setMotionType(int flyType) {
        motionType=flyType;
    }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        return true;
        //        return moveDetector.onTouchEvent(e);
    }
