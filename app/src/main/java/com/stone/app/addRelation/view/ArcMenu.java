package com.stone.app.addRelation.view;
...
public class ArcMenu extends ViewGroup implements View.OnClickListener {
    ...
    private static final int POS_CENTRE = 4;
    int mButtonX = 0;
    int mButtonY = 0;
    //以下5个成员变量是所需要的.
    //声明两个属性 位置 还有半径
    ...
    public interface OnMenuItemClickListener {
        ...
        boolean OnLongClickListener(View view, int pos);
...
//                if (mPosition == Position.CENTER) {
                    ...
                    ct = (int) (mRadius * Math.cos(Math.PI * 2 / (count - 1) * i));
//                } else {
...
//            }
            ...
            if (mCurrentStatus == Status.CLOSE) {
                tranAnim = new TranslateAnimation(startX, endX, startY, endY);
                //当卫星菜单打开的时候,按钮就可以进行点击.
                childView.setClickable(true);
                childView.setFocusable(true);

            } else {
                tranAnim = new TranslateAnimation(endX, startX, endY, startY);
            ...
            childView.setOnLongClickListener(new OnLongClickListener() {
                                                 ...
                                                 public boolean onLongClick(View view) {
                                                     if (mMenuItemClickListener != null) {
                                                         mMenuItemClickListener.OnLongClickListener(childView, pos);
                                                     }
                                                     return false;
                                                 }
                                             }
    ...
    private void menuItemAnim(int pos) {
        for (int i = 0; i < getChildCount() - 1; i++) {
            ...
            //其他子菜单则变小消失.
            if (i == pos) {
                childView.startAnimation(scaleBigAnim(500));
            } else {
                childView.startAnimation(scaleSmallAnim(500));
            }
